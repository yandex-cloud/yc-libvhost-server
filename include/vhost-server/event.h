 ...
 * Events detected in given event loop iteration will also be handled in this thread.
 *
 * Event loop management operations (add/remove events) are thread-safe,
 * although changes to list of events may not be visible until next vhd_run_event_loop.
 */
 ...
 * @timeout     0 to return immediately, -1 to block indefinitely, milliseconds value otherwise.
 *
 * @return      Number of events handled in this run or 0 if we have been interrupted or timed out.
 ...
 *
 * After calling this function vhd_run_event_loop should (eventually) return and will
 * always immediately return 0 on subsequent run attempts (but it is still safe to attempt to run).
 *
 * vhd_event_loop_terminated will also start to return true.
 * Assumed caller pattern is to poll termination in one thread and terminate from another, i.e.:
 *
 * Thread 1:
 * --------
