...
/*
 * vhd_bdev_io values are always expressed in these units for any
 * vhd_bdev_info->sector_size configuration.
 */
...
struct vhd_bdev_info {
     ...
     * or of VHD_SECTOR_SIZE if sector_size is 0.
     */
    uint32_t block_size;

    /*
     * Logical sector size in bytes, VHD_SECTOR_SIZE is used if
...
static inline uint32_t vhd_blockdev_sector_size(
    const struct vhd_bdev_info *bdev)
{
    return bdev->sector_size ? bdev->sector_size : VHD_SECTOR_SIZE;
}
