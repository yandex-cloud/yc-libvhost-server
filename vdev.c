...
void vhd_vring_dec_in_flight(struct vhd_vring *vring)
{
    ...
    if (vring->started_in_rq) {
        ...
        if (vdev->pte_flush_byte_threshold && !vring->disconnecting) {
            size_t processed_bytes, next_threshold;

            if (catomic_load_acquire(&vdev->pte_flush_pending)) {
                return;
            }

            processed_bytes = catomic_load_acquire(&vdev->num_processed_bytes);
            next_threshold = catomic_load_acquire(&vdev->pte_flush_byte_threshold_next);
...
static void pte_flush_refresh_threshold(struct vhd_vdev *vdev)
{
    ...
    processed_bytes = catomic_load_acquire(&vdev->num_processed_bytes);
    catomic_store_release(&vdev->pte_flush_byte_threshold_next,
                          processed_bytes + vdev->pte_flush_byte_threshold);
}
...
static void flush_vdev_ptes(struct vhd_vdev *vdev)
{
    ...
    processed_bytes = catomic_load_acquire(&vdev->num_processed_bytes);
    if (processed_bytes < vdev->pte_flush_byte_threshold_next) {
        /*
    ...
    VHD_OBJ_INFO(vdev, "performing a PTE flush after processing %zu bytes, "
                 "next flush is at %zu bytes", processed_bytes,
                 vdev->pte_flush_byte_threshold_next);
    ...
    size_t pte_flush_byte_threshold)
{
    ...
    *vdev = (struct vhd_vdev) {
        ...
        .pte_flush_byte_threshold_next = pte_flush_byte_threshold,
